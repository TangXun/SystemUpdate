...
public class SystemUpdateReceiver extends BroadcastReceiver {
    ...
    private static final int SHOW_RESULT_TIME_DELAY = 3000;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equalsIgnoreCase(ConnectivityManager.CONNECTIVITY_ACTION)) {
            ...
            if (status == DownloadInfo.STATE_DOWNLOADING) {
                ...
                Xlog.i(TAG, "ACTION_BOOT_COMPLETED, update complete");
            } else if (status == DownloadInfo.STATE_SDINSTALL_START) {

                Xlog.i(TAG, "ACTION_BOOT_COMPLETED, SD card update complete");
                Util.setAlarm(context, AlarmManager.RTC, Calendar.getInstance().getTimeInMillis()
                        + SHOW_RESULT_TIME_DELAY, Util.Action.ACTION_UPDATE_REPORT);
                resetDescriptionInfo(mDownLoadStatus);
                status = DownloadInfo.STATE_QUERYNEWVERSION;
    ...
    private void showDlReminderNotification(Context context, boolean IsForDL) {
        ...
        if(IsForDL) {
            ...
            notification
            .showDownloadingNotificaton(downloadInfo.getVerNum(), (int) (((double) Util
                    .getFileSize(Util.getPackageFileName(mContext)) / (double) downloadInfo
